<?php
/**
 * Upload ALL remaining Snigel images to Shopware
 * Maps local images to products and uploads missing ones
 */
...
foreach ($products as $product) {
    ...
    $stmt = $pdo->prepare("
        SELECT HEX(p.id) as id, HEX(p.cover) as cover_id
        FROM product p
    ...
    foreach ($localImages as $imageName) {
        ...
        // Check if already exists (by filename without extension)
        ...
        try {
            ...
            $stmt = $pdo->prepare("
                ...
                VALUES (
                    UNHEX(?),
                    (SELECT id FROM version WHERE name = 'live' LIMIT 1),
                    UNHEX(?),
                    (SELECT id FROM version WHERE name = 'live' LIMIT 1),
                    UNHEX(?),
                    ?,
            ...
            ");
            $stmt->execute([$productMediaId, $productId, $mediaId, $position, $now]);
