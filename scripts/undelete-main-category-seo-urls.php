<?php
/**
 * Undelete main category SEO URLs
 */
...
echo "=== Undelete Main Category SEO URLs ===\n\n";

// URLs to fix (from check - these are marked as deleted)
$urlsToFix = [
...
// Find and add the other two (raven-caliber-kit and zielhilfen-optik-zubehoer)
$additionalPaths = ['raven-caliber-kit/', 'zielhilfen-optik-zubehoer/'];
...
foreach ($additionalPaths as $path) {
    ...
    if (isset($seoResponse['body']['data'][0])) {
        $id = $seoResponse['body']['data'][0]['id'];
        $urlsToFix[$id] = $path;
        echo "Found: /$path (ID: $id)\n";
    }
}
...
foreach ($urlsToFix as $urlId => $path) {
    ...
    if ($response['code'] >= 200 && $response['code'] < 300) {
        ...
        echo "✗ Error: /$path - " . substr($error, 0, 80) . "\n";
        $errors++;
    }
}
