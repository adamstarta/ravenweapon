<?php
/**
 * Fix Order Confirmation Email - Product Images (Direct SQL)
 *
            ...
            <tr style="border-bottom: 1px solid #eee;">
                <td style="padding: 15px 10px 15px 0; width: 80px; vertical-align: top;">
                    ...
                    {% if lineItem.cover and lineItem.cover.url %}
                        ...
                        {% elseif coverUrl starts with '//' %}
                            {% set imageUrl = 'https:' ~ coverUrl %}
                        {% elseif coverUrl starts with '/' %}
                            {% set imageUrl = 'https://ortak.ch' ~ coverUrl %}
                        {% else %}
                            {% set imageUrl = 'https://ortak.ch/' ~ coverUrl %}
                        {% endif %}
                    {% endif %}
                    ...
                    <div style="width: 80px; height: 80px; background: linear-gradient(135deg, #f5f5f5 0%, #e8e8e8 100%); border-radius: 4px; display: flex; align-items: center; justify-content: center; border: 1px solid #eee;">
                        <span style="color: #999; font-size: 11px; text-align: center;">Kein Bild</span>
                    </div>
                    {% endif %}
                </td>
...
// Get the template type ID for order_confirmation_mail
...
// Update the template translation
...
if ($result) {
    ...
    echo "✓ Updated $rowCount email template translation(s)\n";
    echo "  - Fixed product image URLs to use absolute paths\n";
    echo "  - Fixed German umlauts (ü, ö, ä, ß)\n";
    echo "  - Updated logo URL\n";
} else {
    echo "Error: Failed to update template\n";
}

echo "\nDone! Clear cache to apply changes.\n";
