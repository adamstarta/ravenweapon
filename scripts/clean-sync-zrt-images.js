/**
 * Clean Sync ZRT (Zerotech) Product Images
 *
 * ONLY affects ZRT-* products from Excel file:
 * 1. Deletes existing product-media links (not the media files themselves)
 * 2. Uploads fresh images from Excel
 *
 * Usage:
 *   node clean-sync-zrt-images.js --dry-run    # Preview what will happen
 *   node clean-sync-zrt-images.js              # Execute cleanup and sync
 */
...
async function linkMediaToProduct(productId, mediaId, position, retry = true) {
    ...
    const res = await httpRequest(`${CONFIG.shopware.apiUrl}/product-media`, {
        method: 'POST',
        headers: { 'Authorization': `Bearer ${token}` },
        body: { productId, mediaId, position }
    });
...
async function main() {
    ...
    console.log('\n╔════════════════════════════════════════════════════════╗');
    console.log('║  CLEAN SYNC ZRT PRODUCT IMAGES                         ║');
    console.log('╚════════════════════════════════════════════════════════╝');
    ...
    // Filter to ZRT-* products only
    products = products.filter(p => p.number && p.number.startsWith(CONFIG.productPrefix));
    console.log(`  ZRT products in Excel: ${products.length}\n`);

    let stats = { processed: 0, cleaned: 0, uploaded: 0, skipped: 0, notFound: 0 };
    ...
    for (const product of products) {
        ...
        for (const img of imageUrls) {
            ...
            if (mediaId) {
                const linked = await linkMediaToProduct(productId, mediaId, img.position);
                if (linked) {
