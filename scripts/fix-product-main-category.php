<?php
/**
 * Fix product Main Category for correct breadcrumbs
 *
 * Problem: Products show breadcrumb "Home / Product Name" because their
 * mainCategory points to wrong/old categories
 *
 * Solution: Delete old mainCategories and create new ones pointing to correct categories
 */
...
echo "=== Fix Product Main Category for Breadcrumbs ===\n\n";
...
foreach ($categoryMapping as $categoryId => $categoryName) {
    ...
    foreach ($products as $product) {
        ...
        // First, delete any existing mainCategory for this product in this sales channel
        $existingResult = apiRequest('POST', '/search/main-category', [
            'filter' => [
        ...
        $existingMainCategories = $existingResult['body']['data'] ?? [];

        // Delete existing main categories
        foreach ($existingMainCategories as $existing) {
            $deleteResult = apiRequest('DELETE', '/main-category/' . $existing['id'], null, $token, $config);
            if ($deleteResult['code'] < 300) {
                echo "  Deleted old mainCategory for: $productName\n";
            }
        }

        // Create new mainCategory pointing to the correct category
        $createResult = apiRequest('POST', '/main-category', [
            'productId' => $productId,
        ...
        if ($createResult['code'] < 300) {
            echo "  OK: $productName -> $categoryName\n";
            $totalUpdated++;
        } else {
            $error = $createResult['body']['errors'][0]['detail'] ?? 'Unknown error';
