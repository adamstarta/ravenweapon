<?php
/**
 * Upload Snigel product images to Shopware CHF site (Server-side version - Fixed)
 */
...
function uploadMedia($baseUrl, $token, $imagePath, $fileName) {
    // Step 1: Create media entity first
    $mediaId = bin2hex(random_bytes(16));
    $extension = strtolower(pathinfo($fileName, PATHINFO_EXTENSION));
    $baseName = pathinfo($fileName, PATHINFO_FILENAME);
    $uniqueName = $baseName . '-' . substr($mediaId, 0, 8);

    // Get default media folder (for product images)
    $folderResult = apiPost($baseUrl, $token, 'search/media-default-folder', [
        'filter' => [
    ...
    if ($createResult['code'] >= 300 && $createResult['code'] != 204) {
        echo "    Create media failed: " . json_encode($createResult['data']) . "\n";
        return null;
    }
