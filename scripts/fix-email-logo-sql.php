<?php
/**
 * Fix email template logo via direct database update
 */
...
$stmt = $pdo->query("
    SELECT mt.id, mtt.content_html
    FROM mail_template mt
    JOIN mail_template_type mtype ON mt.mail_template_type_id = mtype.id
    LEFT JOIN mail_template_translation mtt ON mt.id = mtt.mail_template_id
...
// Update all translations for this template
$stmt = $pdo->prepare("
    UPDATE mail_template_translation
    SET content_html = ?, content_plain = ?, subject = ?, updated_at = NOW()
    WHERE mail_template_id = ?
");
$result = $stmt->execute([$htmlContent, $plainContent, $subject, $templateId]);

if ($result) {
    $rowCount = $stmt->rowCount();
    echo "SUCCESS! Updated $rowCount translation(s) with embedded logo.\n";
} else {
