<?php
/**
 * Check ALL SEO URLs for main categories - find duplicates
 */
...
echo "=== ALL SEO URLs for Main Categories (checking for duplicates) ===\n\n";
...
foreach ($mainCategories as $catId => $name) {
    ...
    if (isset($seoResponse['data']) && count($seoResponse['data']) > 0) {
        $count = count($seoResponse['data']);
        echo "  Found $count SEO URL(s):\n";
        ...
        foreach ($seoResponse['data'] as $url) {
            ...
            $modified = $attrs['isModified'] ?? false;

            $markers = [];
            if ($canonical) $markers[] = 'CANONICAL';
            if ($deleted) $markers[] = 'DELETED';
            if ($modified) $markers[] = 'MODIFIED';
            $status = $markers ? ' [' . implode(', ', $markers) . ']' : '';

            // Check if uppercase or lowercase
            $hasUppercase = preg_match('/[A-Z]/', $path);
            $caseNote = $hasUppercase ? ' (UPPERCASE)' : ' (lowercase)';

            echo "    /{$path}{$status}{$caseNote}\n";
            echo "      URL ID: {$url['id']}\n";
        }
    } else {
