<?php
/**
 * Fix Order Confirmation Email - Product Images v2
 *
 * Problem: Product images with spaces in filename breaking in emails
 * Solution: URL-encode the image path properly
 */
...
// Updated HTML template with proper URL encoding for image paths
$htmlContent = <<<'HTML'
        ...
        <table style="width: 100%; border-collapse: collapse; margin-bottom: 20px;">
            ...
            <tr style="border-bottom: 1px solid #eee;">
                <td style="padding: 15px 10px 15px 0; width: 80px; vertical-align: top;">
                    ...
                    {# Try to get URL from cover object #}
                    {% if lineItem.cover %}
                    ...
                    {% else %}
                    <div style="width: 80px; height: 80px; background: linear-gradient(135deg, #f5f5f5 0%, #e8e8e8 100%); border-radius: 4px; display: flex; align-items: center; justify-content: center; border: 1px solid #eee;">
                        <span style="color: #999; font-size: 11px; text-align: center;">Bild</span>
                    </div>
                    {% endif %}
                </td>
...
echo "Found template type ID: " . bin2hex($typeId) . "\n";

// Get the mail template ID
...
if ($result) {
    ...
    echo "  - Added URL encoding for spaces in image filenames\n";
    echo "  - Fixed absolute URL construction\n";
} else {
