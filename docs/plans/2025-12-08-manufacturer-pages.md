...
**Goal:** Create automatic manufacturer listing pages at root-level URLs like `/magpul`, `/lockhart-tactical` that display all products from that manufacturer with filtering.

**Architecture:** Custom controller with catch-all route that slugifies manufacturer names and matches against URL slugs. Uses Shopware-first routing (manufacturer routes only activate when no other route matches). Reuses existing product card and filter styling from category pages.

**Tech Stack:** PHP 8.1+, Symfony Routing, Shopware 6.7, Twig, JavaScript (client-side filtering)

---

## Task 1: Create ManufacturerPageController

**Files:**
- Create: `shopware-theme/RavenTheme/src/Controller/ManufacturerPageController.php`
...
Run: `ls -la shopware-theme/RavenTheme/src/Controller/`
Expected: `ManufacturerPageController.php` exists alongside `LegalPagesController.php`

---

## Task 2: Register Controller in services.xml

**Files:**
- Modify: `shopware-theme/RavenTheme/src/Resources/config/services.xml`

**Step 1: Add controller service definition**

Add after the existing `LegalPagesController` service:
...
**Step 2: Verify services.xml is valid**

Run: `cat shopware-theme/RavenTheme/src/Resources/config/services.xml`
Expected: Both `LegalPagesController` and `ManufacturerPageController` services defined

---

## Task 3: Create Manufacturer Listing Template

**Files:**
- Create: `shopware-theme/RavenTheme/src/Resources/views/storefront/page/manufacturer/index.html.twig`
...
Run: `mkdir -p shopware-theme/RavenTheme/src/Resources/views/storefront/page/manufacturer`

**Step 3: Verify template exists**

Run: `ls -la shopware-theme/RavenTheme/src/Resources/views/storefront/page/manufacturer/`
Expected: `index.html.twig` exists

---

## Task 4: Update Product Cards to Link to Manufacturer Pages

**Files:**
- Modify: `shopware-theme/RavenTheme/src/Resources/views/storefront/component/product/card/box-standard.html.twig`

**Step 1: Find the manufacturer name display and make it a link**

Find this pattern in the file where manufacturer name is displayed and wrap it with a link:

```twig
{# Change manufacturer name from plain text to link #}
{% set manufacturerSlug = product.manufacturer.translated.name|lower|replace({' ': '-', '_': '-'})|replace({'/[^a-z0-9-]/': ''}) %}
<a href="/{{ manufacturerSlug }}" style="font-weight: 700; color: #374151; text-decoration: none;">
...
**Step 2: Test manufacturer URLs**

Navigate to:
- `https://ortak.ch/magpul` - Should show Magpul products
- `https://ortak.ch/lockhart-tactical` - Should show Lockhart Tactical products
- `https://ortak.ch/acheron` - Should show Acheron products

**Step 3: Test that existing routes still work**

Navigate to:
- `https://ortak.ch/agb` - Should show AGB page (not manufacturer)
- `https://ortak.ch/impressum` - Should show Impressum page
- Category pages should still work

**Step 4: Verify filters work**

On manufacturer page:
- Test price filter
- Test stock filter
...
```bash
git add shopware-theme/RavenTheme/src/Controller/ManufacturerPageController.php
git add shopware-theme/RavenTheme/src/Resources/config/services.xml
git add shopware-theme/RavenTheme/src/Resources/views/storefront/page/manufacturer/index.html.twig
```

**Step 2: Commit**

```bash
git commit -m "feat: add automatic manufacturer listing pages

- Add ManufacturerPageController with catch-all route
- Create manufacturer listing template with filters (price, stock, category)
- Use low-priority route to not conflict with existing pages
- Slugify manufacturer names for clean URLs (/magpul, /lockhart-tactical)
...
| File | Action | Purpose |
|------|--------|---------|
| `ManufacturerPageController.php` | Create | Handle `/{slug}` routes, load products |
| `services.xml` | Modify | Register controller with dependencies |
| `page/manufacturer/index.html.twig` | Create | Product listing template |
| `box-standard.html.twig` | Modify | Link manufacturer names to pages |

**URL Examples:**
- `/magpul` → Shows all Magpul products
- `/lockhart-tactical` → Shows all Lockhart Tactical products
- `/zero-tolerance` → Shows all Zero Tolerance products

**Filters Available:**
- Preis (price range)
- Lagerstatus (in stock only)
- Kategorie (filter by product category)
